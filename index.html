      ...
      <div class="grid gap-6 px-6 py-8 md:grid-cols-[1.3fr_0.7fr] md:px-10 md:py-10">
        <div>
          ...
          <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
            一個單頁式的網頁版記帳網站，整合儀表板、表單、歷史清單與理財建議。
          </p>
        </div>
    ...
    <main class="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
        ...
        <section class="glass-card rounded-3xl border border-white/70 p-5 shadow-xl shadow-slate-900/5 sm:p-6">
          ...
          <div class="grid gap-5 lg:grid-cols-2">
            <article class="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div class="h-72"><canvas id="expenseCategoryChart"></canvas></div>
            </article>
            <article class="rounded-2xl border border-slate-200 bg-white p-4">
              <div class="mb-3 flex items-center justify-between">
                <h3 class="font-semibold text-slate-900">每月收支趨勢</h3>
              </div>
              <div class="h-72"><canvas id="monthlyTrendChart"></canvas></div>
            </article>
          </div>
        </section>
        ...
        <section class="glass-card rounded-3xl border border-white/70 p-5 shadow-xl shadow-slate-900/5 sm:p-6">
          <div class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-slate-500">快速新增一筆收入或支出。</p>
            </div>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <label class="block">
                <span class="mb-1 block text-sm font-medium text-slate-700">每月總預算</span>
                <input id="budgetInput" type="number" min="0" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none" placeholder="例如 30000" />
              </label>
              <button id="saveBudgetBtn" type="button" class="mt-auto rounded-2xl bg-slate-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-slate-700">儲存預算</button>
            </div>
          </div>
          ...
          <form id="transactionForm" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <label class="block">
              ...
              <input id="dateInput" type="date" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none" />
            </label>
            <label class="block">
              <span class="mb-1 block text-sm font-medium text-slate-700">流向</span>
              <select id="typeInput" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none">
            ...
            <label class="block">
              ...
              <select id="categoryInput" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none"></select>
            </label>
            <label class="block">
              <span class="mb-1 block text-sm font-medium text-slate-700">金額</span>
              <input id="amountInput" type="number" min="0" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none" placeholder="0" />
            </label>
            <label class="block md:col-span-2 xl:col-span-2">
              <span class="mb-1 block text-sm font-medium text-slate-700">備註</span>
              <input id="noteInput" type="text" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none" placeholder="例如：午餐、薪資入帳" />
            </label>
            <div class="flex items-end">
              <button type="submit" class="w-full rounded-2xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-amber-300">儲存紀錄</button>
            </div>
          </form>
        </section>
      </section>
      ...
      <aside class="space-y-6">
        <section class="glass-card rounded-3xl border border-white/70 p-5 shadow-xl shadow-slate-900/5 sm:p-6">
          ...
          <div class="mt-5 max-h-[540px] space-y-3 overflow-auto pr-1" id="historyList"></div>
        </section>

        <section class="glass-card rounded-3xl border border-white/70 p-5 shadow-xl shadow-slate-900/5 sm:p-6">
          <h2 class="text-xl font-semibold text-slate-900">理財建議</h2>
          <p id="insightsText" class="mt-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900"></p>
        </section>
      </aside>
    </main>
  </div>
